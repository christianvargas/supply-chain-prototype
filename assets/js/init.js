...
$("#wrapper").on('click', '.open-left', function(e){
	e.stopPropagation();
...
$("#sidebar-menu a").on('click',function(e){
  // if( true || $("#wrapper").hasClass("enlarged")){
    e.preventDefault();
...
$(document).on("click", ".widget-header .widget-toggle", function(event){
  ...
  $('html,body').animate({
    scrollTop: $(this).offset().top - 75 - parseFloat($('.topbar-secondary:first').outerHeight())
  }, 1000);
});

$(document).on("click", ".widget-toggle-link", function(event){
  event.preventDefault();
  $('.widget[data-id="'+$(this).data('target')+'"]').find(".widget-content").slideToggle(function(){
    $(event.target).find('i').toggleClass('ti-plus').toggleClass('ti-minus');  
  });
...
$.fn.editableform.buttons =
  '<div class="margin-top">' +
    '<span style="line-height:2em;">After saving you may edit the comment by clicking on the text</span>' +
    '<button type="submit" class="btn btn-sm btn-round btn-white editable-submit pull-right">Add Comment</button>' +
  '</div>'
;
